...
def positive_int(value):
    ...
    if ivalue <= 0:
        raise argparse.ArgumentTypeError("invalid positive_int value: %s" %
                                         value)
...
def parse_comm_line_params():
    ...
    parser.add_argument(
        "problem_type",
        choices=['TP', 'WP', 'DCL', 'XORSAT', 'K_LOCAL'],
    ...
    parser.add_argument(
        "config_file",
        help="Configuration file containing problem-type-specific parameters")

    parser.add_argument(
        "-n",
        help="The number of problem instances to be generated (default: 10)",
        type=positive_int,
        dest="no_of_instances",
        metavar="num_instances",
        default=10)
    parser.add_argument("-o",
                        help="Output format: ising/hobo (default: ising)",
                        dest="output_format",
    ...
    parser.add_argument("-f",
                        help="File format: txt/json (default: txt)",
                        dest="file_format",
                        metavar="file_format",
                        choices=["txt", "json"],
                        default="txt",
...
def read_config_file(params):
    ...
    if params['problem_type'] == 'TP':
        ...
        if params['TP']['dimension'] == 2:
            try:
                params['TP']['tile_params'] = np.array([
                    float(config['TP']['p1']),
                    float(config['TP']['p2']),
                    float(config['TP']['p3'])
                ])

                if not ((params['TP']['tile_params'] >= 0.0).all() and
                        (params['TP']['tile_params'] <= 1.0).all()):
                    raise ValueError
        ...
        if params['TP']['dimension'] == 3:
            try:
                params['TP']['tile_params'] = np.array(
                    [float(config['TP']['pF22']),
                     float(config['TP']['pF42'])])
...
# Apply guage transform?
        try:
            ...
            params['TP']['gauge_transform'] = config['TP'].getboolean(
                'gauge_transform')
        except ValueError:
...
# Discretize couplers?
        try:
            ...
            params['WP']['discretize_couplers'] = config['WP'].getboolean(
                'discretize_couplers')
        except ValueError:
...
# Apply guage transform?
        try:
            ...
            params['WP']['gauge_transform'] = config['WP'].getboolean(
                'gauge_transform')
        except ValueError:
        ...
        if not err_occurred:
            ...
            percent_diff = abs(
                (alpha_provided - alpha_actual)) / alpha_provided * 100.0

            if percent_diff > 10:
                print(
                    'Warning: Note that the difference between provided and used values differs by more than 10%'
                )
        ...
        try:
            ...
            params['DCL']['lambda'] = int(
                temp_lambda) if temp_lambda.is_integer() else temp_lambda
        ...
        if not err_occurred:
            ...
            try:
                params['K_LOCAL']['subproblem_list'] = [
                    val.strip() for val in config['K_LOCAL']
                    ['subproblem_id_list'].split(',')
                ]

                if len(params['K_LOCAL']
                       ['subproblem_list']) < (params['K_LOCAL']['k'] + 1) // 2:
                    raise ValueError

                params['K_LOCAL']['subproblem_list'] = params['K_LOCAL'][
                    'subproblem_list'][:(params['K_LOCAL']['k'] + 1) // 2]
            except ValueError:
                ...
                err_msg += 'For k_max = {}, provide {} subproblem identifiers separated by commas.\n'.format(
                    params['K_LOCAL']['k'], (params['K_LOCAL']['k'] + 1) // 2)
                err_msg += '{} subproblems should be 2-local, and {} subproblems should be 1-local.\n\n'.format(
                    required_2_local_subprobs, required_1_local_subprobs)
        ...
        if not err_occurred:
            params['K_LOCAL']['subproblem_ids'] = list(
                Counter(params['K_LOCAL']['subproblem_list']).keys())
            params['K_LOCAL']['subproblem_count'] = list(
                Counter(params['K_LOCAL']['subproblem_list']).values())

            params['K_LOCAL']['subproblem_types'] = []

            for pId in params['K_LOCAL']['subproblem_ids']:
                try:
                    params['K_LOCAL']['subproblem_types'].append(
                        config[pId]['subproblem_type'].upper())

                    if not (config[pId]['subproblem_type']
                            in ['TP', 'WP', 'RF']):
                        raise ValueError
                except ValueError:
        ...
        if not err_occurred:
            subproblem_localities = [
                1 if sub_type == 'RF' else 2
                for sub_type in params['K_LOCAL']['subproblem_types']
            ]

            if np.sum(
                    np.array(subproblem_localities) *
                    np.array(params['K_LOCAL']['subproblem_count'])
            ) != params['K_LOCAL']['k']:
                ...
                err_msg += 'For k_max = {}, {} subproblems should be 2-local, and {} subproblems should be 1-local.\n\n'.format(
                    params['K_LOCAL']['k'], required_2_local_subprobs,
                    required_1_local_subprobs)
                err_occurred = True
        ...
        if not err_occurred:
            ...
            for subproblem_id, subproblem_type, subproblem_count in zip(
                    params['K_LOCAL']['subproblem_ids'],
                    params['K_LOCAL']['subproblem_types'],
                    params['K_LOCAL']['subproblem_count']):
                subproblem_params = {}
                ...
                if subproblem_type == 'RF':
                    ...
                    try:
                        subproblem_params['dimension'] = int(
                            config[subproblem_id]['dimension'])
                    ...
                    try:
                        subproblem_params['length'] = int(
                            config[subproblem_id]['L'])

                        if subproblem_params[
                                'length'] % 2 != 0 or subproblem_params[
                                    'length'] < 4:
                            raise ValueError
                    except ValueError:
                    ...
                    if subproblem_params['dimension'] is not None:
                        if subproblem_params['dimension'] == 2:
                            try:
                                subproblem_params['tile_params'] = np.array([
                                    float(config[subproblem_id]['p1']),
                                    float(config[subproblem_id]['p2']),
                                    float(config[subproblem_id]['p3'])
                                ])

                                if not ((subproblem_params['tile_params'] >=
                                         0.0).all() and
                                        (subproblem_params['tile_params'] <=
                                         1.0).all()):
                                    raise ValueError
                        ...
                        if subproblem_params['dimension'] == 3:
                            try:
                                subproblem_params['tile_params'] = np.array([
                                    float(config[subproblem_id]['pF22']),
                                    float(config[subproblem_id]['pF42'])
                                ])
                    ...
                    try:
                        ...
                        subproblem_params['gauge_transform'] = config[
                            subproblem_id].getboolean('gauge_transform')
                    ...
                    if not err_occurred:
                        system_sizes.append(subproblem_params['length']**
                                            subproblem_params['dimension'])

                elif subproblem_type == 'WP':

                    try:
                        subproblem_params['length'] = int(
                            config[subproblem_id]['N'])
                    ...
                    try:
                        subproblem_params['alpha'] = float(
                            config[subproblem_id]['alpha'])
                    ...
                    try:
                        ...
                        subproblem_params['discretize_couplers'] = config[
                            subproblem_id].getboolean('discretize_couplers')
                    except ValueError:
                    ...
                    if not err_occurred:
                        ...
                        percent_diff = abs(
                            (alpha_provided -
                             alpha_actual)) / alpha_provided * 100.0
                ...
                if not err_occurred:
                    params['K_LOCAL']['subproblem_params'].append(
                        subproblem_params)
