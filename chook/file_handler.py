...
def create_dir(params):
    ...
    if params['problem_type'] == 'TP':
        ...
        dir_name = 'wishart_planting_N_{}_alpha_{:.2f}'.format(
            params['WP']['length'], params['WP']['alpha'])
        ...
        dir_name = 'regular_xorsat_k_{}_N_{}'.format(params['XORSAT']['k'],
                                                     params['XORSAT']['N'])

    elif params['problem_type'] == 'K_LOCAL':
        dir_name = 'combined_{}_local_N_{}'.format(
            params['K_LOCAL']['k'], params['K_LOCAL']['total_spins'])
    else:
...
def write_text_file(path_to_file, records):
    ...
    with open(path_to_file, "w") as the_file:
        ...
        for row in records:
            writer.writerow([
                '{:.7e}'.format(entry) if isinstance(entry, float) else entry
                for entry in row
            ])
